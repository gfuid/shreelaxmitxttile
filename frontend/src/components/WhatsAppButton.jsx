...
const WhatsAppButton = ({
  phoneNumber = '919394512326',
  defaultMessage = 'Hello Sri Vijay Laxmi Textiles, I would like to inquire about wholesale saree catalogues.',
}) => {
  ...
  return (
    <div className="fixed bottom-20 left-4 md:bottom-6 md:left-6 z-40 flex items-center gap-3">
      {/* WhatsApp Trigger Button with Ripple */}
      <button
        onClick={handleOpenWhatsApp}
        onMouseEnter={() => setShowTooltip(true)}
        onMouseLeave={() => setShowTooltip(false)}
        className="relative w-14 h-14 rounded-full bg-[#25D366] hover:bg-[#20BD5A] text-white shadow-2xl hover:scale-110 active:scale-95 transition-all duration-300 flex items-center justify-center border-2 border-white/90 group cursor-pointer"
        aria-label="Chat with Sri Vijaylaxmi on WhatsApp"
        title="Chat on WhatsApp"
      >
        {/* Soft Radial Ping Wave */}
        <span className="absolute inset-0 rounded-full bg-[#25D366] ping-radial pointer-events-none opacity-40"></span>

        {/* Official SVG WhatsApp Logo */}
        <svg
          viewBox="0 0 32 32"
          className="w-7 h-7 fill-current drop-shadow relative z-10 group-hover:rotate-6 transition-transform duration-300"
